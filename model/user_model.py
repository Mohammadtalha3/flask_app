...
class user_model():
    ...
    def user_addone(self,data):
        self.mycursor.execute(f"INSERT INTO users (name,email,phone,role,password) VALUES('{data['name']}','{data['email']}','{data['phone']}','{data['role']}','{data['password']}')")
        return make_response({'message':"user created successfuly"},201)
    
    def user_update(self,data):
        self.mycursor.execute(f"UPDATE users SET name='{data['name']}', email='{data['email']}',phone='{data['phone']}',role='{data['role']}',password='{data['password']}' WHERE id= {data['id']}")
